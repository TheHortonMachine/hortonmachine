 ...
 */
public class ErmBasinChartAction extends AbstractErmVariableChartAction {
    private static final long serialVersionUID = 1L;

    public ErmBasinChartAction( ADb db, Component parent ) {
        super("Open Basin Data Chart", db, parent);
    }

    @Override
    protected List<ErmEntityItem> loadEntities() throws Exception {
        return ErmBasinChartDataLoader.loadBasinsWithData(db);
    }

    @Override
    protected ErmVariableChartData loadData( int entityId ) throws Exception {
        return ErmBasinChartDataLoader.load(db, entityId);
    }
    ...
    protected String noDataMessage() {
        return "No basin has data in the '" + ErmSchema.BASIN_DATA_TABLE + "' table.";
    }
}
