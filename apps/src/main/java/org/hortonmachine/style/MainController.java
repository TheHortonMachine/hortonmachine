...
public class MainController extends MainView implements IOnCloseListener, TreeSelectionListener {
	...
	private void init() {
		...
		List<String> supportedExtensions = Stream
				.of(HMConstants.SUPPORTED_VECTOR_EXTENSIONS, HMConstants.SUPPORTED_RASTER_EXTENSIONS)
				...
				if (selectedFiles != null && selectedFiles.length > 0) {
					...
					try {
						if (HMConstants.isVector(selectedFile)) {
							...
							featureCollectionFieldNames = FeatureUtilities
									.featureCollectionFieldNames(currentFeatureCollection);
							...
							mapContent.getViewport()
									.setCoordinateReferenceSystem(raster.getCoordinateReferenceSystem());
	...
	private void zoomToSubItems() {
		if (currentFeaturesList != null) {
			...
			mapPane.setDisplayArea(
					new ReferencedEnvelope(env, simpleFeature.getFeatureType().getCoordinateReferenceSystem()));
		}
		...
		if (currentSelectedSW != null) {
			...
			AbstractAction action = new AbstractAction("Add new FeatureTypeStyle") {
				...
				public void actionPerformed(ActionEvent e) {
					...
					tmpName = WrapperUtilities
							.checkSameNameFeatureTypeStyle(styleWrapper.getFeatureTypeStylesWrapperList(), tmpName);
