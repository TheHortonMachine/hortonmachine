			...
			if (selectedFile.getName().endsWith(".asc") || selectedFile.getName().endsWith(".tiff")) {
				...
				try {
					if (parentFolderName.equals(fileName)) {
						ImageMosaicNwwLayer imageMosaicNwwLayer = new ImageMosaicNwwLayer(selectedFile, null, null, false);
				...
				if (RasterizedSpatialiteLasLayer.isLasDb(db)) {
					...
					RasterizedSpatialiteLasLayer rasterizedSpatialiteLayer = new RasterizedSpatialiteLasLayer(name, db,
							null, true, doIntensity);
					...
					if (_useRasterizedCheckbox.isSelected()) {
						...
						if (geometryColumn != null) {
							SpatialiteGeometryType geomType = SpatialiteGeometryType
									.forValue(geometryColumn.geometry_type);
		...
		if (_useRasterizedCheckbox.isSelected()) {
			...
			if (GeometryUtilities.isPolygon(geometryDescriptor)) {
				FeatureCollectionPolygonLayer featureCollectionPolygonLayer = new FeatureCollectionPolygonLayer(name,
						readFC, featureStore, field2ValuesMap);
				...
				FeatureCollectionPointsLayer featureCollectionPointsLayer = new FeatureCollectionPointsLayer(name,
						readFC, featureStore, field2ValuesMap, imagePath);
