...
import org.hortonmachine.geoscript.ConsoleView
  ...
  -cp,-classpath,--classpath <path>    Specify classpath'''
    ...
    static frameConsoleDelegates = [
            rootContainerDelegate:{
                frame(
                    ...
                    defaultCloseOperation: JFrame.DISPOSE_ON_CLOSE, // TODO DO_NOTHING_ON_CLOSE,
                ) {
                    try {
    ...
    boolean askToSaveFile() {
        ...
        switch (JOptionPane.showConfirmDialog(frame,
            ...
            myTitle, JOptionPane.YES_NO_CANCEL_OPTION))
        {
    ...
    void showAbout(EventObject evt = null) {
        ...
        pane.setMessage('Welcome to the Geoscript Groovy Console for evaluating Geoscript and Hortonmachine scripts\n' + myVersions + version)
        def dialog = pane.createDialog(frame, 'About ' + myTitle)
        dialog.show()
    }
    ...
    void updateTitle() {
        if (frame.properties.containsKey('title')) {
            if (scriptFile != null) {
                frame.title = scriptFile.name + (dirty?' * ':'') + ' - ' + myTitle
            } else {
                frame.title = myTitle
            }
        }
...
class GroovyFileFilter extends FileFilter {
    ...
    public String getDescription() {
        "Geoscript and Groovy Source Files ($GROOVY_SOURCE_EXT_DESC)"
    }
