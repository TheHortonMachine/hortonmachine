...
public class Whetgeo1DInputsHandler {
     ...
     *
     * <p>For each known table, creates the table in the db if it does not exist,
     * then populates it from the corresponding CSV file. Timestamps are stored
     * as Unix epoch seconds (UTC).
     *
    ...
    private static void populateDictionary(File csv, ADb db) throws Exception {
        ...
        if (!db.hasTable(tableName)) {
            db.createTable(tableName,
                    COL_DICTIONARY_VAL1 + " TEXT",
                    COL_DICTIONARY_VAL2 + " TEXT");
        }
        if (db.getCount(tableName) > 0) return;
    ...
    private static void populateInputGrid(File csv, ADb db) throws Exception {
        ...
        if (!db.hasTable(tableName)) {
            db.createTable(tableName,
                    ...
                    COL_GRID_ETA + " REAL",
                    COL_GRID_K + " INTEGER",
                    COL_GRID_EQUATION_STATE_ID + " INTEGER",
        ...
        String sql = String.format("""
                ...
                COL_GRID_TYPE, COL_GRID_ETA, COL_GRID_K,
                COL_GRID_EQUATION_STATE_ID, COL_GRID_PARAMETER_ID);
        db.execOnConnection(conn -> {
            boolean autoCommit = conn.getAutoCommit();
            conn.setAutoCommit(false);
            try (IHMPreparedStatement pStmt = conn.prepareStatement(sql)) {
                for (int i = 1; i < lines.size(); i++) {
                    ...
                    try{
                        ...
                        if(!parts[3].trim().equals("nan") && !parts[4].trim().equals("nan")) {
                            throw new IllegalArgumentException("Error parsing line " + (i+1) + " in " + csv.getName() + ": " + line, e);
                        }
                    }
                    pStmt.addBatch();
                }
    ...
    private static void populateRichardsParameters(File csv, ADb db) throws Exception {
        ...
        if (!db.hasTable(tableName)) {
            db.createTable(tableName,
                    COL_RICHARDS_ID + " INTEGER PRIMARY KEY",
                    COL_RICHARDS_THETAS + " REAL",
                    COL_RICHARDS_THETAR + " REAL",
                    COL_RICHARDS_N + " REAL",
                    COL_RICHARDS_ALPHA + " REAL",
                    COL_RICHARDS_ALPHA_SPECIFIC_STORAGE + " REAL",
                    COL_RICHARDS_BETA_SPECIFIC_STORAGE + " REAL",
     ...
     *
     * <p>Reads the @H header line to determine column names. The timestamp column
     * is stored as Unix epoch seconds (UTC); value columns are stored as REAL.
     */
    private static void populateTemperature(File csv, String tableNameStr, ADb db) throws Exception {
        ...
        fieldDefs[0] = COL_TEMPERATURE_TIMESTAMP + " INTEGER";
        ...
        db.execOnConnection(conn -> {
            ...
            conn.setAutoCommit(false);
            try (IHMPreparedStatement pStmt = conn.prepareStatement(sql)) {
                for (int r = 0; r < epochSeconds.size(); r++) {
                    pStmt.setLong(1, epochSeconds.get(r));
