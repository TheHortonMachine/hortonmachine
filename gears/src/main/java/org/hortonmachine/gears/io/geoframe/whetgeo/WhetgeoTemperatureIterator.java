 ...
 *
 * <p>
 * Reads {@code bufferSize} rows at a time from the DB. No boxing, no per-row
 * object allocation.
 *
 * <pre>{@code
...
public class WhetgeoTemperatureIterator implements AutoCloseable {
	...
	WhetgeoTemperatureIterator(ADb db, String tableNameStr, String timestampCol, int bufferSize) throws Exception {
		this(db, tableNameStr, timestampCol, null, null, bufferSize);
	}
	 ...
	 *                    limit
	 */
	WhetgeoTemperatureIterator(ADb db, String tableNameStr, String timestampCol, Long startMillis, Long endMillis,
			int bufferSize) throws Exception {
		SqlName tableName = SqlName.m(tableNameStr);
		StringBuilder sql = new StringBuilder("SELECT * FROM ").append(tableName.fixedDoubleName);
