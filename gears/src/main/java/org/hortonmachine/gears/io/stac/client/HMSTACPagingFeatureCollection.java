...
class HMSTACPagingFeatureCollection extends PagingFeatureCollection {

    private final HTTPClient http;

    public HMSTACPagingFeatureCollection(
    ...
    protected SimpleFeatureCollection readNext(ObjectNode next) throws IOException {
        ...
        try (InputStream is = http.get(new URL(href.textValue())).getResponseStream();
                HMSTACGeoJSONReader reader = new HMSTACGeoJSONReader(is, http)) {
