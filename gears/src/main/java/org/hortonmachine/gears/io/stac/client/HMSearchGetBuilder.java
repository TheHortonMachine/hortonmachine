...
class HMSearchGetBuilder {

    private static final Collector<CharSequence, ?, String> COMMA_JOINER = Collectors.joining(",");

    STACLandingPage landingPage;

    public HMSearchGetBuilder(STACLandingPage landingPage) {
    ...
    URL toGetURL(SearchQuery search) throws MalformedURLException, URISyntaxException {
        ...
        if (search.getIntersects() != null) {
            try {
                builder.addParameter("intersects", HMSTACClient.OBJECT_MAPPER.writeValueAsString(search.getIntersects()));
