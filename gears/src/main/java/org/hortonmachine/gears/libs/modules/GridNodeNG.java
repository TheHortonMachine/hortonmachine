 ...
 * @since 0.11.0
 */
public class GridNodeNG extends NodeNG {
    ...
    public GridNodeNG (HMRaster raster, int col, int row) {
        super(raster, col, row);
        
        xRes = raster.getXRes();
        yRes = raster.getYRes();

        novalue = raster.getNovalue();
        if (isInRaster(col, row)) {
            elevation = raster.getValue(col, row);
            if (raster.isNovalue(elevation)) {
                isValid = false;
            } else {
                isValid = true;
            }
    ...
    public boolean isPitFor( List<GridNodeNG> existingConnectedNodes ) {
        if (!isValid || touchesBound || touchesNovalue) {
            return false;
        }
        boolean isPitFor = false;
        double min = Double.POSITIVE_INFINITY;
        for( GridNodeNG checkNode : existingConnectedNodes ) {
        ...
        if (!doCircular) {
            for( int c = -delta; c <= delta; c++ ) {
                ...
                for( int r = -delta; r <= delta; r++ ) {
                    int tmpRow = row + r;
                    GridNodeNG n = new GridNodeNG(raster, tmpCol, tmpRow);
            ...
            for( int c = -delta; c <= delta; c++ ) {
                ...
                for( int r = -delta; r <= delta; r++ ) {
                    ...
                    if (distance <= radius) {
                        ...
                        window[r + delta][c + delta] = novalue;
                    }
                }
            }
        }
     ...
     */
    public List<GridNodeNG> getWindow( int size ) {
        if (size % 2 == 0) {
            size++;
        }
        List<GridNodeNG> windowNodes = new ArrayList<>(size * size);
        int delta = (size - 1) / 2;
        for( int c = -delta; c <= delta; c++ ) {
            int tmpCol = col + c;
            for( int r = -delta; r <= delta; r++ ) {
     ...
     */
    public int getFlow() {
        GridNodeNG nextDown = goDownstreamSP();
        if (nextDown == null) {
    ...
    /**
     * Gets all surrounding {@link GridNodeNG nodes}, starting from the most eastern.
     * 
     ...
     */
    public List<GridNodeNG> getSurroundingNodes() {
        List<GridNodeNG> nodes = new ArrayList<GridNodeNG>();
        Direction[] orderedDirs = Direction.getOrderedDirs();
     ...
     * @return the node.
     */
    public GridNodeNG getNodeAt( Direction direction ) {
        int newCol = col + direction.col;
        int newRow = row + direction.row;
        GridNodeNG node = new GridNodeNG(raster, newCol, newRow);
        return node;
     ...
     */
    public Direction isNeighborOf( GridNodeNG otherNode ) {
        Direction[] orderedDirs = Direction.getOrderedDirs();
        for( int i = 0; i < orderedDirs.length; i++ ) {
            Direction direction = orderedDirs[i];
            int newCol = col + direction.col;
     ...
     */
    public Direction isSameValueNeighborOf( GridNodeNG otherNode ) {
    ...
    /**
     * Gets only the valid surrounding {@link GridNodeNG nodes}, starting from the most eastern.
     * 
     * @return the valid nodes surrounding the current node. 
     */
    public List<GridNodeNG> getValidSurroundingNodes() {
        if (nodes == null) {
            nodes = new ArrayList<GridNodeNG>();
            Direction[] orderedDirs = Direction.getOrderedDirs();
            for( int i = 0; i < orderedDirs.length; i++ ) {
                Direction direction = orderedDirs[i];
                int newCol = col + direction.col;
    ...
    /**
     * Gets all surrounding {@link GridNodeNG nodes} that <b>DO</b> flow into this node by steepest path rule.
     * 
     * @return the nodes that flow into this node.
     */
    public List<GridNodeNG> getEnteringNodesSP() {
        List<GridNodeNG> nodes = new ArrayList<GridNodeNG>();
        List<GridNodeNG> surroundingNodes = getSurroundingNodes();
        for( GridNodeNG flowNode : surroundingNodes ) {
            if (flowNode != null) {
                GridNodeNG downstream = flowNode.goDownstreamSP();
    ...
    /**
     * Gets all surrounding {@link GridNodeNG nodes} that do <b>NOT</b> flow into this node by steepest path rule.
     * 
     * @return the nodes that flow into this node.
     */
    public List<GridNodeNG> getNonEnteringNodesSP() {
        List<GridNodeNG> nodes = new ArrayList<GridNodeNG>();
        List<GridNodeNG> surroundingNodes = getSurroundingNodes();
        for( GridNodeNG flowNode : surroundingNodes ) {
            if (flowNode != null) {
     ...
     * @return the slope.
     */
    public double getSlopeTo( GridNodeNG node ) {
        double slope = (elevation - node.elevation) / getDistance(node);
        return slope;
    }

    public double getDistance( GridNodeNG node ) {
    ...
    public boolean equals( Object obj ) {
        ...
        if (getClass() != obj.getClass())
            return false;
        GridNodeNG other = (GridNodeNG) obj;
