...
public class OmsZonalStats extends HMModel {
	...
	public void process() throws Exception {
		...
		CoordinateReferenceSystem vectorCrs = bounds.getCoordinateReferenceSystem();
		CoordinateReferenceSystem rasterCrs = inRaster.getCoordinateReferenceSystem();

		boolean crsWereDifferent = !HMCrsRegistry.crsEquals(vectorCrs, rasterCrs);
		if (crsWereDifferent) {
			inVector = new ReprojectingFeatureCollection(inVector, rasterCrs);
		}

		SimpleFeatureBuilder featureBuilder = createFeatureBuilder(rasterCrs, hasUserTotalMean);

		DefaultFeatureCollection tmpOutVector = new DefaultFeatureCollection();
		...
		for (Geometry geometry : geometriesList) {
			...
			tmpOutVector.add(feature);
			if (crsWereDifferent) {
				outVector = new ReprojectingFeatureCollection(tmpOutVector, vectorCrs);
			} else {
				outVector = tmpOutVector;
			}
			pm.worked(1);
		}
		pm.done();
	 ...
	 *                        is thrown in case of cusps, else an errormessage is
	 *                        given, but processing will go on ignoring the
	 *                        geometry.
	 * @return
	 * @throws Exception
	 */
	public static double[] polygonStats(Geometry geometry, GridGeometry2D gridGeometry, RandomIter inIter,
			double novalue, boolean hasUserTotalMean, double[] tm_utm_tac, double percentageThres,
			IHMProgressMonitor monitor) throws Exception {
					...
					if (coords.length == 2) {
						...
						if (coords.length == 1) {
							...
							throw new ModelsIOException(
									MessageFormat.format("Found intersection with more than 2 points in: {0}/{1}",
		...
		if (hasUserTotalMean) {
			...
			result = new double[] { min, max, mean, var, sdev, sum, meanAbsoluteDeviation, activeCellCount,
					passiveCellCount };
		} else {
	...
	protected void processCell(int readCol, int readRow, int writeCol, int writeRow, int readCols, int readRows,
