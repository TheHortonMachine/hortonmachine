...
final class RenderedSampleDimension extends GridSampleDimension {
    ...
    static boolean create(
        ...
        if (src != null && src.length != numBands) {
            throw new IllegalArgumentException(
                            ErrorKeys.NUMBER_OF_BANDS_MISMATCH_$3.formatted(
                            numBands,
                            src.length,
                            "SampleDimension"));
        }
    ...
    private static void create(
        ...
        if (min != null && min.length != numBands) {
            throw new IllegalArgumentException(
                            ErrorKeys.NUMBER_OF_BANDS_MISMATCH_$3.formatted( numBands, min.length, "min[i]"));
        }
        if (max != null && max.length != numBands) {
            throw new IllegalArgumentException(
                            ErrorKeys.NUMBER_OF_BANDS_MISMATCH_$3.formatted( numBands, max.length, "max[i]"));
        }
