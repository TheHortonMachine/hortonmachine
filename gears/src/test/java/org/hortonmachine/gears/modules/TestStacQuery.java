...
public class TestStacQuery extends HMTestCase {
	...
	// This example shows the data of different queries are being added to each
	// other.
	// In this context that operation makes no sense, as this query is a landcover
	// raster where each
	// value has a meaning (1 - water, 2 - trees...)
	// My question is if the HortonMachine would be able to apply different actions
	// to the rasters
	...
	// System.out.println("(" + i + "," + j + "):" + rasterOne.getValue(i, j) + ":"
	// + rasterThree.getValue(i, j));
	// totalCells++;
	...
	// }
	// }
	// System.out.println("Percentage of coincidence: " + numberOfEqualCells * 100.0
	// / totalCells + "%");
	// }
	...
	private static HMRaster getRaster(Date start, Date end) throws Exception {
		...
		try (HMStacManager stacManager = new HMStacManager(catalogURL, pm)) {
			...
			Geometry polygon = r.read(
					"POLYGON ((-3.0521 42.8042, -3.0521 43.3908, -1.2943 43.3908, -1.2943 42.8042, -3.0521 42.8042))");
			...
			HMRaster raster = collection.readRasterBandOnRegion(latLongRegionMap, asset -> asset.getId().equals("data"),
					items, true, MergeMode.AVG, pm);
