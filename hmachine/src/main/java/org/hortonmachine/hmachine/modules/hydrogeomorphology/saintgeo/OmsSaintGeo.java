...
public class OmsSaintGeo extends HMModel {
	...
	// @Description("Lateral discharge tribute or offtake section progressive value
	// for each id.")
	// @In
	...
	// @Description("Lateral immission from confluences progressive value for each
	// id.")
	// @In
	...
	@Description("The output results for each timestep and section. Ensured to be ordered by timestep.")
	@Out
	public Map<Integer, List<Result>> outResultsMap;
	...
	public static final String pKs_Description = "Gaukler-Strickler coefficient for the evaluation of the steady flow initial condition.";
	...
	private Integer SCELTA_A_MONTE;
	private Integer SCELTA_A_VALLE;
	...
	public void process() throws Exception {
		...
		/*
		 * Upstream, two conditions are possible 1 discharge over time 2 level elevation
		 * over time
		 */
		if (SCELTA_A_MONTE == null)
			SCELTA_A_MONTE = 1;

		// Downstream, three conditions are possible
		// 1 level elevation over time
		// 2 transition to critical flow
		// 3 transmissive conditio
		if (SCELTA_A_VALLE == null)
			SCELTA_A_VALLE = 2;
		DELT = pDeltaTMillis / 1000.0;

		if (pKs != null) {
			pm.message("Using user defined Gaukler-Strickler coefficient for all sections that have no value: " + pKs);
		}
		...
		RiverInfo riverInfo = ARiverSectionsExtractor.getRiverInfo(riverPointsFeatures, sectionFeatures,
				sectionPointsFeatures, pKs);
		...
		outResultsMap = new TreeMap<>();

		/*
		 * The length of the simulation is taken from the input head discharge this is
		 * also the reference for all the other input data that have to cover the same
		 * interval with the same timesteps.
		 */
		for (int timeIndex = 0; timeIndex < inDischarge.length; timeIndex++) {
			boolean isLast = timeIndex == inDischarge.length - 1;
			...
			if (waterLevel == null) {
				...
				/*
				 * assign the initial discharge in all the sections with the value of the first
				 * head discharge
				 */

				for (int i = 0; i < sectionsCount - 1; i++) {
					discharge[i] = qHead;
					DELXM[i] = riverPoints.get(i + 1).getProgressiveDistance()
							- riverPoints.get(i).getProgressiveDistance();
				}

				/*
				 * Evaluation of the steady flow condition using the initial discharge (first
				 * value of the head discharge) using the Gaukler-Strickler formula
				 */
				...
				/*
				 * The condition of steady flow is reached when the maximum difference between
				 * the water levels calculated in two subsequent instants is less than the
				 * tollerance TOL_mu
				 */
				int conta_cicli = 0;
				...
				while (error >= (TOL_mu / 10.0) && conta_cicli <= 60000) {
					...
					new_tirante(riverPoints, waterLevel, discharge, celerity, DELXM, SCELTA_A_MONTE, qHead,
				...
				pm.message("Number of cicles used to reach the steady flow condition: " + conta_cicli);
			...
			/*
			 * calculate the new water level: new_tirante generate the tridiagonal system
			 * for the evaluation of the new water level
			 */
			...
			for (int i = 0; i < sectionsCount - 1; i++) {
				...
				result.progressiveDistance = section.getProgressiveDistance();
				result.waterLevel = waterLevel[i];
				result.minSectionElevation = minsez;
				result.rightBankElevation = sectionCoordinates[dx].z;
				result.leftBankElevation = sectionCoordinates[sx].z;
				result.froudeNumber = froudeNumber;
				result.discharge = discharge[i] < 0.0 ? 0.0 : discharge[i];
				result.celerity = celerity[i] < 0.0 ? 0.0 : celerity[i];
				result.wettedArea = idrgeo[i][0];
				outResultsMap.computeIfAbsent(timeIndex, k -> new ArrayList<>()).add(result);
				
				if (isLast) {
					linkId2LevelMap.put(sectionId, result.waterLevel);
					linkId2RelativeLevelMap.put(sectionId, (result.waterLevel - result.minSectionElevation));
					linkId2DischargeMap.put(sectionId, result.discharge);
					linkId2VelocityMap.put(sectionId, result.celerity);
				}
			}
			RiverPoint section1 = riverPoints.get(sectionsCount - 1);
			Coordinate[] sectionCoordinates1 = section1.getSectionCoordinates();
			int sectionId1 = section1.getSectionId();
			double minsez1 = section1.getMinElevation();
			int dx1 = section1.getStartNodeIndex();
			int sx1 = section1.getEndNodeIndex();
			...
			result1.sectionId = sectionId1;
			result1.progressiveDistance = section1.getProgressiveDistance();
			result1.waterLevel = waterLevel[sectionsCount - 1];
			result1.minSectionElevation = minsez1;
			result1.rightBankElevation = sectionCoordinates1[dx1].z;
			result1.leftBankElevation = sectionCoordinates1[sx1].z;
			result1.froudeNumber = froudeNumber;
			double lastDischarge = discharge[sectionsCount - 2] < 0.0 ? 0.0 : discharge[sectionsCount - 2];
			result1.discharge = lastDischarge;
			...
			result1.celerity = lastCelerity;
			result1.wettedArea = idrgeo[sectionsCount - 1][0];
			outResultsMap.computeIfAbsent(timeIndex, k -> new ArrayList<>()).add(result1);
			if (isLast) {
				linkId2LevelMap.put(sectionId1, result1.waterLevel);
				linkId2RelativeLevelMap.put(sectionId1, (result1.waterLevel - result1.minSectionElevation));
				linkId2DischargeMap.put(sectionId1, result1.discharge);
				linkId2VelocityMap.put(sectionId1, result1.celerity);
			}
...
//			outputDischargeWriter.write(sbDischarge.toString() + "\n");
		...
		if(outputLevelFile != null && outputDischargeFile != null) {
			try (BufferedWriter outputLevelWriter = new BufferedWriter(new FileWriter(outputLevelFile));
				...
				for (var entry: outResultsMap.entrySet()) {
					...
					for (Result result : results) {
						sbLevel.append(result.sectionId).append(";");
						sbLevel.append(result.progressiveDistance).append(";");
						sbLevel.append(result.waterLevel).append(";");
						sbLevel.append(result.minSectionElevation).append(";");
						sbLevel.append(result.rightBankElevation).append(";");
						sbLevel.append(result.leftBankElevation).append("\n");
					}
					outputLevelWriter.write(sbLevel.toString() + "\n");
					...
					sbDischarge.append("\n#timestep: " + timeIndex + "\n");
					for (Result result : results) {
						sbDischarge.append(result.sectionId).append(";");
						sbDischarge.append(result.progressiveDistance).append(";");
						sbDischarge.append(result.froudeNumber).append(";");
						sbDischarge.append(result.discharge).append(";");
						sbDischarge.append(result.celerity).append(";");
						sbDischarge.append(result.wettedArea).append("\n");
					}
					outputDischargeWriter.write(sbDischarge.toString() + "\n");
				}
			}
	...
	private void calculateGauklerStrickler(double q, double[] level, List<RiverPoint> sectionsList) {
		...
		for (int i = 0; i < imax; i++) {
			...
			if (i == 0 || i == 1)
				IF = (minsez[i] - minsez[i + 1]) / (sectionsList.get(i + 1).getProgressiveDistance()
						- sectionsList.get(i).getProgressiveDistance());
			else if (i == imax - 1 || i == imax - 2)
				IF = (minsez[i - 1] - minsez[i]) / (sectionsList.get(i).getProgressiveDistance()
						- sectionsList.get(i - 1).getProgressiveDistance());
			else
				IF = (minsez[i - 2] - minsez[i + 2]) / (sectionsList.get(i + 2).getProgressiveDistance()
						- sectionsList.get(i - 2).getProgressiveDistance());
			if (IF <= 0)
				IF = (minsez[0] - minsez[imax - 1]) / (sectionsList.get(imax - 1).getProgressiveDistance()
						- sectionsList.get(0).getProgressiveDistance());
			/*
			 * the function is calculated for the extreme values looking for the minimun
			 * value at the bank
			 */
			max_tir = maxsez[i];
			tir_dx = max_tir;
			...
			while (toll >= TOLL && conta_cicli <= MAX_CICLI) {
				...
				if ((val_dx * val_sx) > 0) {
					pm.errorMessage(
							"Evaluation of the steady flow not possible for the section " + i + "solution not found.");
				}
	 ...
	 * <p>
	 * <b>wetted area</b> calculated as the sum of the trapezoids obtained by
	 * tracing a vertical line starting from each points of the sections; each
	 * trapezoid has a base on the right (base_dx) and a base on the left (base_sx)
	 * and an height (altezza). <b>wetted perimeter</b> as the sum of all the wetted
	 * segments of the river ground <b>hydraulic radius</b> using the definition
	 * formula <b>width of water surface</b> height of the trapezoids defined to
	 * calculate the wetted area <b>roughness coefficient</b> using the Engelund
	 * method: the section is divided into trapezoids as for the wetted area and for
	 * each trapezoid this quantity is calculated: <b>Ks(j)*Y(j)^(5/3)*B(j)</b>
	 * where:
	 * </p>
	 * <ul>
	 * <li><b>Ks(j)</b> Gaukler-Strickler coefficient for the segment j</li>
	 * <li><b>Y(j)</b> water depth for the trapezoid j obtained as the ratio between
	 * the area and the width of the water surface for each trapezoid</li>
	 * <li><b>B(j)</b> is the width of the water surface of the trapezoid j</li>
	 * </ul>
	 * <p>
	 * NOTE: the hydraulic radius of the trapezoid j is approximated with the water
	 * depth even if this is recommended only with large sections.
	 * </p>
	 * <p>
	 * The <b>roughness coefficient</b> is obtained dividing the sum of all the
	 * quantities with <b>(A*RH^(2/3))</b>, where A is the total wetted area and RH
	 * the hydraulic radius referred to the whole section.
	 * </p>
	 * <p>
	 * <b>The alpha coefficient of Coriolis</b> is calculated using the same
	 * subdivision of the section used for the calculation of the wetted area and
	 * for each trapezoid this quantity is evaluated:
	 ...
	 * @return una matrice che ha come elementi di ogni colonna della i-esima riga
	 *         le grandezze: - wetted area - wetted perimeter - hydraulic radius -
	 *         roughness - alpha coefficient of Corilis relative alla i-esima
	 *         sezione.
	 */
	private double[][] wettedArea(double[] waterLevel, List<RiverPoint> riverPoints) {
		...
		/*
		 * Calculate: wetted area, wetted perimeter, width of the water surface, the
		 * roughness and the alpha coefficient of Coriolis in the section i
		 */
		for (int i = 0; i < imax; i++) {
			...
			for (int j = (int) dx - 1; j < sx - 1; j++) {
				 ...
				 * 
				 * // if (section.getYAt(j) >= tirante[i] && section.getYAt(j + 1) >=
				 * tirante[i]) { // area_b = area_b + 0; // peri_b = peri_b + 0; // larghe_b =
				 * larghe_b + 0; // gau_b = gau_b + 0; // alfa_num = alfa_num + 0; // alfa_den =
				 * alfa_den + 0; // } /* case 1: only partially wetted (right side dry)
				 */
				...
				if (sectionCoordinates[j].z >= waterLevel[i] && sectionCoordinates[j + 1].z < waterLevel[i]) {
					...
					peri_b = peri_b
							+ Math.sqrt(Math.abs(base_dx - base_sx) * Math.abs(base_dx - base_sx) + altezza * altezza);
					/* the width of the water surface */
					larghe_b = larghe_b + altezza;
					/* the roughness coefficient */
					gau_b = gau_b
							+ (sectionGauklerStrickler * Math.pow((base_dx + base_sx) / 2.0, (5.0 / 3.0)) * altezza);
				...
				if (sectionCoordinates[j + 1].z < waterLevel[i] && sectionCoordinates[j].z < waterLevel[i]) {
					...
					larghe_b = larghe_b + altezza;
					gau_b = gau_b
							+ (sectionGauklerStrickler * Math.pow((base_dx + base_sx) / 2, (5.0 / 3.0)) * altezza);
					gau_loc = sectionGauklerStrickler;
	...
	private void new_tirante(List<RiverPoint> riverPoints, double[] tirante, double[] Q, double[] U, double[] DELXM,
		...
		/*
		 * Execute the method wettedArea the variable idrgeo contains all the quantities
		 * related to water depth and section
		 */
		double[][] idrgeo = wettedArea(tirante, riverPoints);
		/*
		 * goemid contains all the quantities related to intermediate sections, linear
		 * interpolation between the values calculated for the given sections
		 */
		...
		for (int i = 0; i < imax - 1; i++) {
			...
			// height of the point outside the bank on the left for the current and for the
			// next
			// section
			...
			if (T1 > A1sx && T2 > A2sx) {
				...
				c = (T2 - T1 - A2sx + A1sx) / l;
				qs[i] = qs[i]
						+ (Cq / (2.5 * c)) * (Math.pow((T2 - A2sx), (5.0 / 2.0)) - Math.pow((T1 - A1sx), (5.0 / 2.0)));
		...
		/*******************************************************************************************
		 * Define the coefficient of the matrix and the denominate number considering
		 * the upstream and downstream assigned conditions.
		 * ******************************************************************************************
		...
		if (SCELTA_A_MONTE == 1 && SCELTA_A_VALLE == 1) {
			...
			DS[0] = -C1 / dx;
			D[0] = C1 / dx + base / DELT;
			B[0] = (base / DELT) * tirante[0] - F_Q[0] / (dx * (1.0 + DELT * GAM[0])) + qin / dx + ql[imax - 2]
					- qs[imax - 2];
			...
			B[imax - 2] = base / DELT * tirante[imax - 2] + Ci / dx * tiranteout
					- F_Q[imax - 2] / (dx * (1.0 + DELT * GAM[imax - 2]))
					+ F_Q[imax - 3] / (dx * (1.0 + DELT * GAM[imax - 3])) + ql[imax - 2] - qs[imax - 2];
			/* Move the values of the water height at time n in the vector tirante_old[] */
			for (int i = 0; i < imax; i++)
				tirante_old[i] = tirante[i];
			/*
			 * The function ris_sistema calculates the values of the water height at time
			 * n+1 and save them in the vector tirante[]
			 */
			linearAlgebra.ris_sistema(D, DS, DI, B, tirante, imax - 1);

			/*
			 * Check on the water depth: if during the elaborations the height of the water
			 * depth is less than the minimum, this minimum value is assigned
			 */
			for (int i = 0; i < imax; i++) {
		...
		if (SCELTA_A_MONTE == 1 && SCELTA_A_VALLE == 3) {
			...
			/*
			 * TODO: add the calculation of water depth and velocity and the check on the
			 * water depth (min_value)
			 */
		}
		...
		if (SCELTA_A_MONTE == 2 && SCELTA_A_VALLE == 1) {
			...
			for (int i = 2; i < imax - 2; i++) {
				...
				B[i] = (idrgeo[i][3] / DELT - Ci - C_old) * tirante[i] + Ci * tirante[i + 1] + C_old * tirante[i - 1]
						- F_Q[i] / (2.0 * dx * (1.0 + DELT * GAM[i]))
						+ F_Q[i - 1] / (2.0 * dx * (1.0 + DELT * GAM[i - 1])) - (Q[i] - Q[i - 1]) / (2.0 * dx);
			}
			...
			B[imax - 2] = (-C_old - Ci + (idrgeo[imax - 2][3]) / DELT) * tirante[imax - 2]
					+ Ci * (tiranteout + tirante[imax - 1]) + C_old * tirante[imax - 3]
					- F_Q[imax - 2] / (2.0 * dx * (1.0 + DELT * GAM[imax - 2]))
					+ F_Q[imax - 3] / (2.0 * dx * (1.0 + DELT * GAM[imax - 3]))
	 ...
	 * @param U
	 * @param idrgeo      containing all the values for the sections i, i+1, ... at
	 *                    time n;
	...
	private void FQ(double[] F_Q, double[] Q, double[] U_I, double[] U, double[][] idrgeo, List<RiverPoint> riverPoints,
		...
		if ((u_i) >= 0) {
			coeff = delta_T
					/ ((riverPoints.get(1).getProgressiveDistance() - riverPoints.get(0).getProgressiveDistance()));
			F_Q[0] = q_i - coeff * (alfa_i * u_i * q_i - alfa * u * q);
		} else {
			coeff = 2 * delta_T
					/ ((riverPoints.get(2).getProgressiveDistance() - riverPoints.get(0).getProgressiveDistance()));
		...
		for (int i = 1; i < imax - 2; i++) {
			...
			alfa = 1;
			if ((u_i) >= 0) {
				coeff = 2 * delta_T / ((riverPoints.get(i + 1).getProgressiveDistance()
						- riverPoints.get(i - 1).getProgressiveDistance()));
				F_Q[i] = q_i - coeff * (alfa_i * u_i * q_i - alfa * u * q);
			} else {
				coeff = 2 * delta_T / ((riverPoints.get(i + 2).getProgressiveDistance()
						- riverPoints.get(i).getProgressiveDistance()));
		...
		q_ii = qout;
		alfa_ii = 1;
		coeff = delta_T / ((riverPoints.get(imax - 1).getProgressiveDistance()
				- riverPoints.get(imax - 2).getProgressiveDistance()));
		u = U[imax - 3];
		q = Q[imax - 3];
		alfa = 1;
		if ((u_i) >= 0) {
			coeff = 2 * delta_T / ((riverPoints.get(imax - 1).getProgressiveDistance()
					- riverPoints.get(imax - 3).getProgressiveDistance()));
	...
	public static class Result {
		...
		public double minSectionElevation;
		public double leftBankElevation;
		public double rightBankElevation;
