...
public class Tca3d extends JGTModel {
	...
	public void process() throws Exception {
		...
		WritableRaster pitWR = CoverageUtilities
				.renderedImage2WritableRaster(pitfillerRI, false);
	...
	private WritableRaster area3d(WritableRaster pitImage, WritableRaster flowImage,
			WritableRaster tca3dImage) {
		...
		for (int j = 1; j < rows - 1; j++) {
			for (int i = 1; i < cols - 1; i++) {
				...
				if (!isNovalue(pitAtIJ)) {
					...
					for (int k = 1; k <= 8; k++) {
						double pitAtK0 = pitIter.getSampleDouble(i
								+ dir[tri[k][0]][0], j + dir[tri[k][0]][1], 0);
						double pitAtK1 = pitIter.getSampleDouble(i
								+ dir[tri[k][1]][0], j + dir[tri[k][1]][1], 0);
