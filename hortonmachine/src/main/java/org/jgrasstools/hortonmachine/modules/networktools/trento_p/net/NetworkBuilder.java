...
public class NetworkBuilder implements Network {
	...
	public static class Builder {
		 ...
		 * @param msg
		 */
		public Builder(IJGTProgressMonitor pm, Pipe[] networkPipe, double n,
				double a, double[][] diameters,
				SimpleFeatureCollection inPipeFC,
				SimpleFeatureCollection outPipeFC, StringBuilder strBuilder) {
			this.pm = pm;
			this.n = n;
			this.a = a;
			...
			if (two[i] == 1) {
				...
				while (networkPipes[k].getIdPipeWhereDrain() != 0) {
					...
					if (networkPipes[oldk].finalFreesurface < networkPipes[k].initialFreesurface) {
						double delta = -networkPipes[oldk].finalFreesurface
								+ networkPipes[k].initialFreesurface;
			...
			if (two[i] == 1) {
				...
				while (networkPipes[k].getIdPipeWhereDrain() != 0) {
					...
					if (networkPipes[oldk].finalFreesurface < networkPipes[k].initialFreesurface) {
						 ...
						 * giunzione
						 */
						delta = -networkPipes[oldk].finalFreesurface
								+ networkPipes[k].initialFreesurface;
						/*
	...
	public void geoSewer() throws Exception {
		...
		while (magnitude[k] == 1) {
			...
			if (networkPipes[l - 1].getAverageResidenceTime() >= 0) {
				 ...
				 */
				networkPipes[l - 1].residenceTime = ((HOUR2MIN
						* networkPipes[l - 1].getAverageResidenceTime() * pow(
						networkPipes[l - 1].getDrainArea() / METER2CM, exponent))

				/ (pow(networkPipes[l - 1].getRunoffCoefficient(), esp) * pow(
				 ...
				 * L * i ^ GAMMA [ min ]
				 */
				networkPipes[l - 1].residenceTime = (-networkPipes[l - 1]
						.getDrainArea()
						* networkPipes[l - 1].getAverageResidenceTime() / networkPipes[l - 1]
						.getLenght());
			}
			...
			for (j = 0; j <= jMax; j++) {
				...
				No = networkPipes[l - 1].getLenght()
						/ (MINUTE2SEC * networkPipes[l - 1].residenceTime
								* celerityfactor * networkPipes[l - 1].meanSpeed);
				...
				r = RootFindingFunctions.rtbis(rfFunction, inf, sup, tolerance,
						jMax, pm);
				 ...
				 * s * ha ]
				 */
				networkPipes[l - 1].coeffUdometrico = (networkPipes[l - 1]
						.getRunoffCoefficient()
						* a
						* pow(networkPipes[l - 1].tP, n - 1)
						* (1 + MINUTE2SEC * celerityfactor
								* networkPipes[l - 1].meanSpeed
								* networkPipes[l - 1].tP
								/ networkPipes[l - 1].getLenght() - 1 / No
								* log(exp(No) + exp(r) - 1)) * 166.6666667);
				/*
				 * Portata Q [ l / s ]
				 */
				networkPipes[l - 1].discharge = (networkPipes[l - 1].coeffUdometrico * networkPipes[l - 1]
						.getDrainArea());

				networkPipes[l - 1].designPipe(diameters, tau, g, maxd, c,
						strBuilder);
			 ...
			 */
			networkPipes[l - 1].tQmax = (networkPipes[l - 1].residenceTime * log(exp(No)
					+ exp(r) - 1));
			/*
		...
		while (k < magnitude.length) {
			...
			if (networkPipes[l - 1].getAverageResidenceTime() >= 0) {
				...
				networkPipes[l - 1].residenceTime = ((HOUR2MIN
						* networkPipes[l - 1].getAverageResidenceTime() * pow(
						...
						networkPipes[l - 1].getRunoffCoefficient(), esp) * pow(
						networkPipes[l - 1].getAverageSlope(), gamma)));
			} else {
			 ...
			 */
			geometry[l - 1][3] = ModelsEngine.varianceDoublematrixColumn(net,
					1, geometry[l - 1][2]);
	...
	private double discharge(int l, double dtp, double[][] net,
			double[] localdelay) throws IOException {
		int indx;
		/*
		...
		for (tp = tpMin; tp <= tpMax; tp += dtp) {
			...
			tpeak = networkPipes[l - 1].residenceTime
					* log(exp(tp / networkPipes[l - 1].residenceTime)
							+ exp(localdelay[l - 1]
									/ networkPipes[l - 1].residenceTime) - 1);
			...
			for (t = tmin; t <= tmax; t += dtp) {
				...
				for (int i = 0; i < net.length; i++) {
					 ...
					 * portata in l.
					 */
					net[i][6] = dischargeFunction(indx, tp, t, net[i][2],
							localdelay);
					/*
		...
		for (int i = 0; i < net.length; i++) {
			...
			net[i][3] = dischargeFunction(indx, networkPipes[l - 1].tP,
					networkPipes[l - 1].tQmax, net[i][2], localdelay);

			if (net[i][3] <= minDischarge) {

				strBuilder.append("Minimum direct discharge in pipe no."
						+ net[i][0] + " at time t=" + t + " for tp=" + tt
		 ...
		 */
		deltaQ = dischargeFunction(l, networkPipes[l - 1].tP,
				networkPipes[l - 1].tQmax, 0, localdelay);
	...
	private double dischargeFunction(int indx, double tp, double t,
			double delay, double[] localdelay) {
		short s = 0;
		/*
		...
		if (t > delay && t <= delay + ext1) {
			...
			return networkPipes[indx - 1].getDrainArea()
					...
					* ((t - delay) / localdelay[indx - 1] + (networkPipes[indx - 1].residenceTime / localdelay[indx - 1])
							* (exp(-(t - delay)
									/ networkPipes[indx - 1].residenceTime) - 1))
					* d;
		}
		...
		if (t > delay + ext2 && t <= delay + ext1 + ext2) {
			...
			return networkPipes[indx - 1].getDrainArea()
					...
					* (1 - (t - delay - tp) / localdelay[indx - 1] + (networkPipes[indx - 1].residenceTime / localdelay[indx - 1])
							* (1 + (1 - exp(localdelay[indx - 1]
									/ networkPipes[indx - 1].residenceTime) - exp(tp
									/ networkPipes[indx - 1].residenceTime))
									* exp(-(t - delay)
											/ networkPipes[indx - 1].residenceTime)))
					* d;
			...
			return networkPipes[indx - 1].getDrainArea()
					...
					* ((networkPipes[indx - 1].residenceTime / localdelay[indx - 1])
							* (1 - exp(tp
									/ networkPipes[indx - 1].residenceTime))
							* (1 - exp(localdelay[indx - 1]
									/ networkPipes[indx - 1].residenceTime)) * exp(-(t - delay)
	...
	private void minMaxT(double[][] net, double[] localdelay, double tp,
			double[] t) {
		/*
		...
		for (int i = 0; i < net.length; i++) {
			...
			tpeak = networkPipes[num].residenceTime
					* log(exp(tp / networkPipes[num].residenceTime)
							+ exp(localdelay[num]
									/ networkPipes[num].residenceTime) - 1);

			if ((tpeak + net[i][2]) < t[0]) {
				t[0] = ModelsEngine.approximate2Multiple((tpeak + net[i][2]),
						tDTp);
			}

			if ((tpeak + net[i][2] + tDTp) > t[1]) {
				t[1] = ModelsEngine.approximate2Multiple(
						(tpeak + net[i][2] + tDTp), tDTp);
			}
		}
	...
	private double scanNetwork(int k, int l, double[] one, double[][] net,
		...
		for (int j = 0; j < k; j++) {
			...
			while (networkPipes[ind - 1].getIdPipeWhereDrain() != 0) {
				 ...
				 */
				t += networkPipes[ind - 1].getLenght()
						/ (celerityfactor * MINUTE2SEC * networkPipes[ind - 1].meanSpeed);
				/*
