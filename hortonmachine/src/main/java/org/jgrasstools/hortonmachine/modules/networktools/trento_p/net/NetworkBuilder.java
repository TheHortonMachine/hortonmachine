...
public class NetworkBuilder implements Network {
     ...
     * </ol>
     * 
     * @param align is a switch that allow to select the metod to allign the
     *        free surface.
     * @param networkPipes is a matrix which contains the networks value.
     * @param two array which contains the ID of the pipe where other pipes
     *        drains.
    ...
    private void resetDepths0( double[] two ) {
        ...
        int[][] upstreampipes = new int[length][Pipe.getMaxJunction() + 1];
     ...
     * 
     * @param networkPipes is a matrix which contains the networks value.
     * @param two Puntatore al vettore che contiene la magnitude delle varie
     *        aree.
     */
    ...
    private void resetDepths1( double[] two ) {
        ...
        upstreamPipes = new int[networkPipes.length][Pipe.getMaxJunction() + 1];
    ...
    public void geoSewer() throws Exception {
        ...
        Utility.pipeMagnitude(magnitude, two, pm);/*
                                                   ...
                                                   * ciascun stato
                                                   */
        double tolerance = Pipe.getTolerance();

        /* al vettore two vengono assegnati gli elementi di magnitude */
        for( int i = 0; i < two.length; i++ ) /*
                                               * al vettore two vengono
                                               * assegnati gli elementi di
                                               * magnitude
                                               */
        {
            two[i] = magnitude[i];
        ...
        int jMax = Pipe.getjMax();
        double c = Pipe.getC();
        double g = Pipe.getG();
        double tau = Pipe.getTau();
        minDischarge = Pipe.getMinDischarge();
     ...
     * <p>
     * 
     * @param warnings2 file di output.
     * @param l Tratto che si sta dimensionando.
     * @param dtp \f$[min]\f$ Passo temporale con cui valutare la portata, per
     *        cercare quella massima.
     * @param net Puntatore alla matrice che contiene tutti i dati del
     ...
     * @param delay [min] ritardo con cui l'onda arriva all'uscita che si sta
     *        dimensionando.
     ...
     * @param localdelay ritardo locale delle tubazioni.
     * @param tp
     * @param t
     */
