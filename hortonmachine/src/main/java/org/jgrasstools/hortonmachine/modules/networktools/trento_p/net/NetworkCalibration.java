...
public class NetworkCalibration implements Network {
    ...
    public static class Builder {
         ...
         * @param dt time step.
         * @param rainData the rain data.
         * @param Qout the output, discharge.
         * @param strBuilder a tring used to store the warnings.
         */
        public Builder( IJGTProgressMonitor pm, Pipe[] networkPipe, double dt, double[][] rainData, double[][] Qout,
    ...
    private void internalPipeVerify( int k, double[] cDelays, double[][] net ) {
        ...
        localdelay = 1;
        double accuracy = Pipe.getAccuracy();
        int jMax = Pipe.getjMax();
        double minG = Pipe.getMinG();
        double maxtheta = Pipe.getMaxTheta();
        ...
        do {
            ...
            Qmax = ModelsEngine.sumDoublematrixColumns(k, qPartial, Qout, 1, qPartial[0].length - 1, pm);
            if (Qmax <= 1)
                Qmax = 1;
            // Resets delays
            for( int i = 0; i < net.length; i++ ) {
     ...
     * @param Qpartial matrice delle portate temporanee necessarie per costriure
     *        l'idrogramma.
