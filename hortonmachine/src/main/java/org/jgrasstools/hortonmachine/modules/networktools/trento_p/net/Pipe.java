...
public class Pipe {
	...
	private static HortonMessageHandler msg = HortonMessageHandler
			.getInstance();
	...
	public void setK(double defaultEsp1, double defaultExponent,
			double defaultGamma) {
		this.k = (HOUR2MIN * averageResidenceTime * pow(drainArea / METER2CM,
				defaultExponent))
				/ (pow(runoffCoefficient, defaultEsp1) * pow(averageSlope,
						defaultGamma));
	 ...
	 */
	private void setKnowNetworkValue(SimpleFeature pipeFeature, int verify)
			throws Exception {
		...
		try {
			 ...
			 */
			this.id = setFeatureField(pipeFeature,
					PipesTrentoP.ID.getAttributeName()).intValue();
			double tmp = setFeatureField(pipeFeature,
					PipesTrentoP.DRAIN_AREA.getAttributeName()).doubleValue();
			if (tmp >= 0.0) {
				this.drainArea = tmp;
			} else {
				pm.errorMessage(msg.message("trentoP.error.fieldArea") + tmp);
				throw new IllegalArgumentException(
						msg.message("trentoP.error.fieldarea" + tmp));
			}
			this.lenght = ((Geometry) pipeFeature.getDefaultGeometry())
					.getLength();
			this.point = ((Geometry) pipeFeature.getDefaultGeometry())
					.getCoordinates();

			this.initialElevation = setFeatureField(pipeFeature,
					PipesTrentoP.INITIAL_ELEVATION.getAttributeName())
					.doubleValue();
			;

			this.finalElevation = setFeatureField(pipeFeature,
					PipesTrentoP.FINAL_ELEVATION.getAttributeName())
					.doubleValue();

			tmp = setFeatureField(pipeFeature,
			...
			if (tmp >= 0) {
				...
				throw new IllegalArgumentException(
						msg.message("trentoP.error.runO" + this.id));
			}
			tmp = setFeatureField(pipeFeature,
			...
			if (tmp >= 0) {
				...
				pm.errorMessage(msg.message("trentoP.error.averageTime")
						+ this.id);
				throw new IllegalArgumentException(
						msg.message("trentoP.error.averageTime" + this.id));
			}
			tmp = setFeatureField(pipeFeature,
					PipesTrentoP.KS.getAttributeName()).doubleValue();
			if (tmp >= 0) {
				this.ks = tmp;
			} else {
				pm.errorMessage(msg.message("trentoP.error.ks") + this.id);
				throw new IllegalArgumentException(
						msg.message("trentoP.error.ks" + this.id));

			}

			this.minimumPipeSlope = setFeatureField(pipeFeature,
					PipesTrentoP.MINIMUM_PIPE_SLOPE.getAttributeName())
					.doubleValue();

			int sectionTmp = setFeatureField(pipeFeature,
			...
			if (sectionTmp > 0 && sectionTmp < 4) {
				...
				throw new IllegalArgumentException(
						msg.message("trentoP.error.section" + this.id));
			}
			this.averageSlope = setFeatureField(pipeFeature,
					PipesTrentoP.AVERAGE_SLOPE.getAttributeName()).intValue();
			;
			if (verify == 1) {
				/* Pipe diameter [cm] */

				this.diameterToVerify = setFeatureField(pipeFeature,
						PipesTrentoP.DIAMETER_TO_VERIFY.getAttributeName())
						.doubleValue();
				/* Pipe slope [%] */

				this.verifyPipeSlope = setFeatureField(pipeFeature,
			...
			throw new Exception(msg.message("trentop.illegalNet"));
	...
	private Number setFeatureField(SimpleFeature pipe, String key) {
		...
		if (field == null) {
			...
			throw new IllegalArgumentException(
					msg.message("trentoP.error.number" + key));
		}
	...
	public void designPipe(double[][] diameters, double tau, double g,
			double maxd, double c, StringBuilder strWarnings) {
	 ...
	 */
	private void designCircularPipe(double[][] diameters, double tau, double g,
			double maxd, StringBuilder strWarnings)

	{
		...
		naturalslope = METER2CM * (getInitialElevation() - getFinalElevation())
				/ getLenght();
		...
		if (naturalslope > pipeSlope) {
			 ...
			 * tratto progettato
			 */
			newtheta = getDiameterI(diameters, naturalslope, g, dD, maxd,
					strWarnings);
		...
		meanSpeed = ((8 * discharge) / (CUBICMETER2LITER * D * D
				* (newtheta - sin(newtheta)) / METER2CMSQUARED));
		/* Quota scavo all'inizio del tubo [m s.l.m.] */
		depthInitialPipe = (getInitialElevation() - minimumDepth - (D + 2 * dD[0])
				/ METER2CM);
		/*
		...
		initialFreesurface = depthInitialPipe + emptyDegree * D / METER2CM
				+ dD[0] / METER2CM;
		/* Quota pelo libero all'inizio del tubo [m s.l.m.] */
		finalFreesurface = depthFinalPipe + emptyDegree * D / METER2CM + dD[0]
				/ METER2CM;
	...
	private double getDiameter(double[][] diameters, double tau, double g,
			double[] dD, double maxd, StringBuilder strWarnings) {
		/* Pari a A * ( Rh ^1/6 ) */
		double B;
		...
		double newrh;
		/* B=A(Rh^1/6) [m^13/6] */
		B = (discharge * sqrt(WSPECIFICWEIGHT / tau))
				/ (CUBICMETER2LITER * getKs());
		/* Angolo formato dalla sezione bagnata [rad] */
		thta = 2 * acos(1 - 2 * g);
		/* Diametro tubo [cm] */
		oldD = TWO_TWENTYOVERTHIRTEEN
				* pow(B, SIXOVERTHIRTEEN)
				/ (pow((1 - sin(thta) / thta), ONEOVERTHIRTEEN) * pow(thta
						- sin(thta), SIXOVERTHIRTEEN));
		/*
		...
		if (oldD < diameters[diameters.length - 1][0]) {
			...
			known = (B * TWO_TENOVERTHREE)
					/ pow(D / METER2CM, THIRTHEENOVERSIX);
			/*
			 * Angolo formato dalla sezione bagnata considerando un diametro
			 * commerciale [rad]
			 */
			newtheta = Utility.thisBisection(thta, known, ONEOVERSIX, minG,
					accuracy, jMax, pm, strWarnings);
			/*
			 ...
			 */
			if (newtheta > thta) {
				strWarnings.append(msg.message("trentoP.warning.bisection")
						+ id);
			}
	 ...
	 *            a string which collect all the warning messages.
	 */
	private double getDiameterI(double[][] diameters, double slope, double g,
			double[] dD, double maxd, StringBuilder strWarnings) {
		int j; /* \param */
		/* Pari a A * ( Rh ^1/6 ) */
		double B;
		 ...
		 * Diametro tubo [ cm ]
		 */
		oldD = TWO_THIRTEENOVEREIGHT
				* pow(B, THREEOVEREIGHT)
				/ (pow((1 - sin(thta) / thta), 0.25) * pow(thta - sin(thta),
						THREEOVEREIGHT));
		...
		if (oldD < diameters[diameters.length - 1][0]) {
			...
			known = (B * TWO_THIRTEENOVERTHREE)
					/ pow(D / METER2CM, EIGHTOVERTHREE);
			/* Riempimento effettivo considerando il diametro commerciale [rad] */
			newtheta = Utility.thisBisection(thta, known, TWOOVERTHREE, minG,
					accuracy, jMax, pm, strWarnings);
	 ...
	 *            a string which collect all the warning messages.
	 */
	private void designRectangularPipe(double tau, double g, double maxd,
			double c, StringBuilder strWarnings) {
		...
		D = diameter;
		/* Velocita media nella sezione [m/s] */
		meanSpeed = (discharge / CUBICMETER2LITER)
	...
	private double getHeight1(double tau, double g, double maxd, double c) {
		...
		oldD = METER2CM * pow(B, SIXOVERTHIRTEEN)
				* pow((2 * g + c), ONEOVERTHIRTEEN)
				/ pow((g * c), SEVENOVERTHIRTEEN);
		/* [cm] base */
		oldb = c * oldD;
		D = ceil(oldD);
		b = c * D;
	...
	private double getHeight1I(double slope, double g, double maxd, double c) {
		...
		/* altezza sezione rettangolare */
		oldD = METER2CM * pow(B, THREEOVEREIGHT) * pow(2 * g + c, ONEOVERFOUR)
				/ pow((g * c), FIVEOVEREIGHT);
		/* [cm] base */
		oldb = c * oldD;
		D = ceil(oldD);
		b = c * D;
	...
	private void designTrapeziumPipe(double tau, double g, double maxd,
			double c, StringBuilder strWarnings) {
		/* [cm] Base della sezione effettivamente adottata. */
		double base;
		/*
	...
	private double getHeight2(double tau, double g, double maxd, double c) {
		...
		oldD = METER2CM * pow(B, SIXOVERTHIRTEEN)
				* pow((c + 2 * sqrt(2) * g), ONEOVERTHIRTEEN)
		...
		/* Grado di riempimento del tubo */
		emptyDegree = ((2 * sqrt(2) * rh * D - D * b) + sqrt(pow((D * b - 2
				* sqrt(2) * rh * D), 2)
				+ 4 * D * D * rh * b))
				/ (2 * D * D);
		/* pendenza del tratto progettato [%] */
	...
	private double getHeight2I(double slope, double g, double maxd, double c) {
		...
		B = (discharge) / (CUBICMETER2LITER * ks * sqrt(slope / METER2CM));
		/* altezza sezione rettangolare */
		oldD = METER2CM * pow(B, THREEOVEREIGHT)
				* pow((c + 2 * sqrt(2) * g), ONEOVERFOUR)
				/ pow((g * (g + c)), FIVEOVEREIGHT);
		/* [cm] base */
		oldb = c * oldD;
		D = ceil(oldD);
		b = c * D;
