...
public class SqlTemplatesAndActions {
    ...
    static {
        ...
        templatesMap.put("unix epoch timestamp select",
        ...
        return new AbstractAction("Show spatial metadata") {
            ...
            public void actionPerformed(ActionEvent e) {
                String query = "SELECT * FROM geom_cols_ref_sys WHERE Lower(f_table_name) = Lower('"
                        + column.parent.tableName + "') AND Lower(f_geometry_column) = Lower('" + column.columnName
                        + "')";
                spatialiteViewer.addTextToQueryEditor(query);
            }
        };
        ...
        return new AbstractAction("Quick view other table") {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    String[] tableColsFromFK = column.tableColsFromFK();
                    String refTable = tableColsFromFK[0];
                    QueryResult queryResult = spatialiteViewer.currentConnectedDatabase.getTableRecordsMapIn(refTable,
                            null, true, 20, -1);
    ...
    public static Action getSelectAction(TableLevel table, SpatialiteViewer spatialiteViewer) {
        return new AbstractAction("Select statement") {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    String query = SpatialiteGuiUtils.getSelectQuery(spatialiteViewer.currentConnectedDatabase, table,
                            false, true);
                    spatialiteViewer.addTextToQueryEditor(query);
        ...
        return new AbstractAction("Reproject table") {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    String[] result = GuiUtilities.showMultiInputDialog(spatialiteViewer, "Reprojection parameters",
                            labels, values, null);
                    ...
                    query += "SELECT RecoverGeometryColumn('" + result[0] + "', '" + geometryColumn.columnName + "',"
                            + result[1] + ",'" + geometryColumn.columnType + "',"
                            + geometryColumn.geomColumn.coord_dimension + ");";
